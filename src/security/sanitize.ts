...
export function sanitizeText(
  ...
  text = text.replace(
    /\b(?:telegram|discord|whatsapp|slack):[^\s,)]+/gi,
    "[channel-id]",
  );
  text = text.replace(
    /\b(?:chat|channel)[_-]?id[:=]\s*-?\d{5,}\b/gi,
    "$1_id=[redacted]",
  );
  text = text.replace(
...
function metadataTextLimit(key: string): {
  ...
  if (
    ...
    return { length: 220, preserveFormatting: false };
  }
  return { length: 80, preserveFormatting: false };
}
...
export function sanitizeMetadata(
  ...
  for (const [key, value] of Object.entries(input)) {
    ...
    if (
      ...
      const limit = metadataTextLimit(key);
      output[key] = sanitizeText(value, limit.length, limit.preserveFormatting);
    }
  }
